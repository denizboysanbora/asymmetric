#!/usr/bin/env python3
"""
APPS Stock Analysis - Late July 2020
...
def get_apps_data():
    """Get APPS data for July 2020"""
    api_key = os.getenv('ALPACA_API_KEY')
    secret_key = os.getenv('ALPACA_SECRET_KEY')
    
    client = StockHistoricalDataClient(api_key, secret_key)
    
    # Get extended data for analysis (starting from March 2020 to ensure enough history)
    request = StockBarsRequest(
        symbol_or_symbols="APPS",
...
def analyze_flag_breakout_potential(bars: List[Bar], target_date, current_price: float, current_volume: float) -> Dict:
    ...
    # OPTIMIZED PARAMETERS (from PRTS and AMD analysis)
...
def analyze_apps_july_2020(bars: List[Bar]):
    """Analyze APPS for late July and early August 2020"""
    
    # Get July and August 2020 bars
    july_bars = [bar for bar in bars if bar.timestamp.month == 7 and bar.timestamp.year == 2020]
    august_bars = [bar for bar in bars if bar.timestamp.month == 8 and bar.timestamp.year == 2020]
    
    # Combine and sort
    all_bars = july_bars + august_bars
    all_bars.sort(key=lambda x: x.timestamp.date())
    
    # Focus on late July + first two weeks of August
    late_july = july_bars[-10:]  # Last 10 trading days of July
    early_august = august_bars[:10]  # First 10 trading days of August
    analysis_period = late_july + early_august
    
    print("🚩 APPS JULY-AUGUST 2020 - LATE JULY + EARLY AUGUST ANALYSIS")
    print("=" * 80)
    ...
    for i, bar in enumerate(analysis_period):
        ...
        if i > 0:
            prev_price = float(analysis_period[i-1].close)
...
def main():
    ...
    print("🔍 APPS July-August 2020 Analysis - Late July + Early August")
    ...
    try:
        ...
        # Analyze APPS July 2020
        results = analyze_apps_july_2020(bars)
        
        print("\n" + "=" * 80)
        print("📋 FINAL SUMMARY:")
        print("=" * 80)
        print("🎯 APPS July-August 2020 - Late July + Early August Analysis")
        print("🔧 Used optimized parameters from PRTS and AMD analysis")
        print("💡 Shows how the algorithm performs on APPS stock")
