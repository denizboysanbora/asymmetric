...
def get_celh_data():
    """Get CELH data for June 2020"""
    api_key = os.getenv('ALPACA_API_KEY')
    secret_key = os.getenv('ALPACA_SECRET_KEY')
    
    client = StockHistoricalDataClient(api_key, secret_key)
    
    # Get extended data for analysis (starting from February 2020 to ensure enough history)
    request = StockBarsRequest(
        symbol_or_symbols="CELH",
...
def analyze_flag_breakout_potential(bars: List[Bar], target_date, current_price: float, current_volume: float) -> Dict:
    ...
    # OPTIMIZED PARAMETERS (from PRTS, AMD, and APPS analysis)
...
def analyze_celh_june_2020(bars: List[Bar]):
    """Analyze CELH for June 2020"""
    
    # Get June 2020 bars
    june_bars = [bar for bar in bars if bar.timestamp.month == 6 and bar.timestamp.year == 2020]
    june_bars.sort(key=lambda x: x.timestamp.date())
    
    print("🚩 CELH JUNE 2020 ANALYSIS")
    print("=" * 80)
    ...
    for i, bar in enumerate(june_bars):
        ...
        if i > 0:
            prev_price = float(june_bars[i-1].close)
...
def main():
    ...
    try:
        ...
        # Analyze CELH June 2020
        results = analyze_celh_june_2020(bars)
        
        print("\n" + "=" * 80)
        print("📋 FINAL SUMMARY:")
        print("=" * 80)
        print("🎯 CELH June 2020 Analysis")
        print("🔧 Used optimized parameters from PRTS, AMD, and APPS analysis")
        print("💡 Shows how the algorithm performs on CELH stock")
