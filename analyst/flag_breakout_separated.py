#!/usr/bin/env python3
"""
Separated Flag Breakout Analysis
- Flag Breakout Potential (setup days): "Flag Breakout?"
- Flag Breakout Day (actual breakout): "Flag Breakout!"
"""
import os
import sys
import numpy as np
...
def detect_flag_breakout_potential(bars: List[Bar], current_date) -> Dict:
    """
    Detect Flag Breakout Potential (setup phase)
    This analyzes the days LEADING UP TO the breakout
    Criteria: Prior impulse + Higher lows + Tight base + ATR contraction
    """
    if len(bars) < 60:
    ...
    # 2. Higher Lows Pattern (last 20 days)
    recent_lows = lows[-20:]
    higher_lows = 0
    ...
    # 3. Tight Base (last 30 days, excluding current day if it's a breakout)
    ...
    # 4. ATR Contraction (last 20 days)
...
def detect_flag_breakout_day(bars: List[Bar], current_price: float, current_volume: float) -> Dict:
    """
    Detect Flag Breakout Day (actual breakout)
    This analyzes the DAY OF the breakout
    Criteria: Price breakout + Volume expansion
    """
    if len(bars) < 60:
    ...
    required_price = recent_high * 1.01  # Relaxed to 1.0% from 1.5%
...
def analyze_july_flag_breakouts(bars: List[Bar]):
    """Analyze July days for separated flag breakout detection"""
    ...
    print("📊 SEPARATED FLAG BREAKOUT ANALYSIS - JULY 2020")
    print("=" * 80)
    print("🎯 Flag Breakout Potential (setup): 'Flag Breakout?'")
    print("🎯 Flag Breakout Day (actual): 'Flag Breakout!'")
    print("=" * 80)
    ...
    for i, bar in enumerate(july_bars):
        ...
        # Check Flag Breakout Potential (setup phase)
        potential_result = detect_flag_breakout_potential(historical_bars, date)
        
        # Check Flag Breakout Day (actual breakout)
        breakout_result = detect_flag_breakout_day(historical_bars, price, volume)
...
def main():
    ...
    print("🔍 Separated Flag Breakout Analysis")
    print("Separating Breakout Potential from Breakout Day")
    ...
    try:
        ...
        # Analyze separated flag breakouts
        analyze_july_flag_breakouts(bars)
        
        print("\n" + "=" * 80)
        print("📋 SEPARATED BREAKOUT SUMMARY:")
        print("=" * 80)
        print("🎯 Flag Breakout Potential: Setup phase criteria")
        print("    • Prior impulse ≥30%")
        print("    • Higher lows ≥40% (relaxed)")
        print("    • Tight base ≤25% (relaxed)")
        print("    • ATR contraction ≤0.9 (relaxed)")
        print("\n🎯 Flag Breakout Day: Actual breakout criteria")
        print("    • Price breakout ≥1.0% above recent high (relaxed)")
