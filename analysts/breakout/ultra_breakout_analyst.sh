...
if [ -f "$LOCK_FILE" ]; then
    ...
    if ps -p "$PID" > /dev/null 2>&1; then
        echo "$(date '+%Y-%m-%d %H:%M:%S') Ultra breakout analyst already running (PID: $PID), skipping..." >> /Users/deniz/Code/asymmetric/analysts/breakout/logs/ultra_breakout_analyst.log
        exit 0
    else
        rm -f "$LOCK_FILE"
    fi
